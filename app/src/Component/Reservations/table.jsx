...
export default function Tableau({ Terrain }) {
  ...
  const handleClick = (heure, dayIndex) => {
    ...
    if(location.pathname === "/Admin"){
      localStorage.setItem("selectedHour", heure);
      localStorage.setItem("selectedTime", dateString);
  ...
  return (
    <div className={`${location.pathname === "/Admin" ? '' : 'mt-6 sm:mt-12 grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8'}`}>
              ...
              {Heures.map((heure, rowIndex) => (
                <tr key={rowIndex} className="hover:bg-gray-700/30 transition-colors duration-150">
                  ...
                  {Jours.map((_, colIndex) => {
                    const reservationState = getReservationState(heure, colIndex); 
                    return (
                      <td
                        key={colIndex}
                      ...
                      >
                        {reservationState === "en attente" || !reservationState ? (
                          <motion.button
                            className={`text-2xs sm:text-xs font-semibold px-1 py-0.5 rounded ${
                              reservationState === "en attente"
      ...
      {sessionStorage.getItem("type") !== "admin" ? (
        <FormResev Terrain={Terrain} selectedHour={selectedHour} selectedTime={selectedTime}  />
      ) : ('')}
    </div>
  );
